...
function listTeamsByCountry() {
    const countryList = teams.map((pais) => ({
        name: pais.name,
...
function sortTeamsByWins() {
    const countryList = teams.map((team) => ({        
        name: team.name,        
        wins: (winsByTeams.find(win => win.teamId === team.id)).wins
    }));

    const equipoFinalOrdenado = countryList.sort(function(a, b) {
        return b.wins - a.wins;
    })
	return equipoFinalOrdenado;
}
...
function leaguesWithWins() {
    return leagues.forEach(l => {
        const teamIds = teamsByLeague
            .filter(team => team.leagueId === l.id) //nos quedamos los equipos de la liga
            .map(team => team.teamId); // y cogemos sus IDs
        console.log('Equipos de la liga', l.id, ':', teamIds.toString()); //corregir undefined
        ...
        console.log('La liga', l.name, 'tiene', wins, 'victorias')
    });
}
...
function newTeamRanking () {
  let newTeam = { id: 14, country: 'Francia', name: 'Everton' };
  let unirTeamLeague = { teamId: 14, leagueId: 7};
  let championsWin = { teamId: 14, wins: 4 };
  
  teams.push(newTeam);
  teamsByLeague.push(unirTeamLeague);
  winsByTeams.push(championsWin);
	...
	const rankingPosition = equipoFinalOrdenado.map((indice) => { return indice.name; }).indexOf('Everton');
...
async function teamUpperCase() {
    return await teams.map(team => team.name.toUpperCase());
}
async function getTeamsNamesAsUpperCase() {
    let response = await teamUpperCase();
